...
if ( ! function_exists( 'st_add_scripts' ) ) {
	...
	function st_add_scripts() {
		...
		wp_register_script('moment', get_template_directory_uri() . '/js/moment.js', [], NULL, TRUE);
		wp_register_script( 'fullcalendar', get_template_directory_uri() . '/js/fullcalendar-5.8.0/lib/main.min.js', [ 'jquery', 'moment' ], null, true );
		...
		$lang      = get_locale(); // Hamzeh
		$lang_file = ST_TRAVELER_DIR . '/js/locales/bootstrap-datepicker.' . $lang . '.min.js';
		wp_enqueue_script( 'bootstrap-datepicker.js', get_template_directory_uri() . '/js/bootstrap-datepicker.js', [ 'jquery', 'moment.min' ], null, true );
		if ( file_exists( $lang_file ) ) {
			wp_enqueue_script( 'bootstrap-datepicker-lang.js', get_template_directory_uri() . '/js/locales/bootstrap-datepicker.' . $lang . '.min.js', [ 'jquery' ], null, true );
		} else {
		...
		wp_enqueue_script('datepicker-persian-plugin-js', get_template_directory_uri() . '/js/datepicker-persian/persian-date.min.js', ['jquery'], null, true); // Hamzeh
		wp_enqueue_script('datepicker-persian-js', get_template_directory_uri() . '/js/datepicker-persian/persian-datepicker.min.js', ['jquery', 'datepicker-persian-plugin-js'], null, true);
        wp_enqueue_style('datepicker-persian-css', get_template_directory_uri() . '/js/datepicker-persian/persian-datepicker.min.css'); // Hamzeh
		...
		$lang      = get_locale();
		$lang_file = ST_TRAVELER_DIR . '/js/locales/bootstrap-datepicker.' . $lang . '.min.js';
		wp_register_script( 'bootstrap-datepicker.js', get_template_directory_uri() . '/js/bootstrap-datepicker.js', [ 'jquery', 'moment.min' ], null, true );// Hamzeh
		if ( file_exists( $lang_file ) ) {
		...
		$mapbox_api_key = st()->get_option('st_token_mapbox'); // hamzeh
		if ($mapbox_api_key != false) {
			if ( is_rtl() ) {
		...
		if ( is_page_template( 'template-user.php' ) ) {
			...
			$google_api_key = st()->get_option('google_api_key'); // hamzeh
			$mapbox_api_key = st()->get_option('st_token_mapbox');
			if ($google_api_key != false) { // hamzeh

			} elseif ($mapbox_api_key != false) { // hamzeh
		...
		if ( is_page() && is_page_template( 'template-user.php' ) ) {
			if ( st()->get_option( 'booking_enable_captcha', 'off' ) == 'on' ) { // hamzeh
			...
			wp_deregister_script('moment');
			wp_register_script('moment', get_template_directory_uri() . '/js/moment.js', [], NULL, TRUE);
			wp_enqueue_script( 'daterangepicker-js', get_template_directory_uri() . '/v2/js/daterangepicker/daterangepicker.js', [ 'jquery', 'moment' ], null, true );
