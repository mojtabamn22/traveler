        ...
        static function showRoomPriceInfo($room_id = '', $check_in = '', $check_out = ''){
            ...
            if(get_post_type($room_id) == 'hotel_room'){
                ...
                foreach($groupday as $key => $date){
                    ...
                    if ( $price_by_per_person == 'on' ) { // hamzeh
                        $html .= '
                        <li><span class="label">'. date_i18n('Y/m/d',strtotime(date(TravelHelper::getDateFormat(), $date[0]))).' - '. date_i18n('Y/m/d',strtotime(date(TravelHelper::getDateFormat(), $date[1]))) .'</span><span class="value">'. TravelHelper::format_money($price_arr['adult_price']) .'</span><span class="value">'. TravelHelper::format_money($price_arr['child_price']) .'</span></li>
					';
                    } else { // hamzeh
                        $html .= '
                        <li><span class="label">'. date_i18n('Y/m/d',strtotime(date(TravelHelper::getDateFormat(), $date[0]))) .' - '. date_i18n('Y/m/d',strtotime(date(TravelHelper::getDateFormat(), $date[1]))) .'</span><span class="value">'. TravelHelper::format_money($price_tmp) .'</span></li>';
                    }
                }
        ...
        static function showRentalPriceInfo($rental_id = '', $check_in = '', $check_out = ''){
            ...
            if(get_post_type($rental_id) == 'st_rental'){
                ...
                if(!empty($custom_price)){
                	if(count($custom_price) == 1){
						$html .= '
                        	<li><span class="label">'. date_i18n('Y/m/d', $date[0]) .' - '. date_i18n('Y/m/d', $date[1]) .'</span><span class="value">'. TravelHelper::format_money($price_tmp) .'</span></li>'; // hamzeh
	                }else{
		                $groupday = STPrice::getGroupDay($check_in, $check_out);
		                foreach($groupday as $key => $date){
			                $status = 'available';
							...
							if ( $price_type == 'person' ) {
								$html .= '
								<li><span class="label">'. date_i18n('Y/m/d', $date[0]) .' - '. date_i18n('Y/m/d', $date[1]) .'</span><span class="value">'. TravelHelper::format_money($price_arr['adult_price']) .'</span><span class="value">'. TravelHelper::format_money($price_arr['child_price']) .'</span></li>';
							} else {
								$html .= '
								<li><span class="label">'. date_i18n('Y/m/d', $date[0]) .' - '. date_i18n('Y/m/d', $date[1]) .'</span><span class="value">'. TravelHelper::format_money($price_tmp) .'</span></li>';
							}
