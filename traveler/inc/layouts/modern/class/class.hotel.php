    ...
    class STHotel extends TravelerObject
    {
        ...
        public function check_price_ajax()
        {
            ...
            if (!HotelHelper::check_day_cant_order($room_origin, $checkin_ymd, $checkout_ymd, $room_num_search, $adult_number, $child_number)) {
                STTemplate::set_message(sprintf(__('This room is not available from %s to %s.', 'traveler'), date_i18n('Y/m/d',strtotime($check_in)), date_i18n('Y/m/d',strtotime($check_out))), 'danger');
                $pass_validate = false;
                return false;
            }
        ...
        function do_add_to_cart()
        {
            ...
            if (!HotelHelper::check_day_cant_order($room_origin, $checkin_ymd, $checkout_ymd, $room_num_search, $adult_number, $child_number)) {
                STTemplate::set_message(sprintf(__('This room is not available from %s to %s.', 'traveler'), date_i18n('Y/m/d',strtotime($checkin_ymd)), date_i18n('Y/m/d',strtotime($checkout_ymd))), 'danger');
                $pass_validate = false;
                return false;
            }
            if (!HotelHelper::_check_room_only_available($room_origin, $checkin_ymd, $checkout_ymd, $room_num_search)) {
            ...
            $data = [
                ...
                'check_in' => st()->get_option('use_woocommerce_for_booking', 'off') == 'on' ? date_i18n('Y/m/d', strtotime($check_in)) : $check_in, // Hamzeh
                'check_out' => st()->get_option('use_woocommerce_for_booking', 'off') == 'on' ? date_i18n('Y/m/d', strtotime($check_out)) : $check_out, // Hamzeh
        ...
        function _get_where_query($where)
        {
            ...
            // Filter by discount only for Hotel (checks if any hotel_room has discount_rate > 0)
            $discount = STInput::get('discount', STInput::get('discount_only', ''));
            if (!empty($discount)) {
                $where .= " AND {$wpdb->prefix}posts.ID IN (
                    SELECT meta1.meta_value FROM {$wpdb->prefix}posts
                    JOIN {$wpdb->prefix}postmeta as meta1 ON {$wpdb->prefix}posts.ID = meta1.post_id AND meta1.meta_key = 'room_parent'
                    WHERE {$wpdb->prefix}posts.post_type = 'hotel_room'
                    AND {$wpdb->prefix}posts.ID IN (
                        SELECT post_id FROM {$wpdb->prefix}postmeta
                        WHERE (meta_key = 'discount_rate' AND meta_value != '' AND CAST(meta_value AS DECIMAL) > 0)
                    )
                )";
            }
            ...
            if ($check = STInput::get('st_orderby')) {
                ...
                switch ($check) {
                    ...
                    case "price_desc":
                        ...
                        break;
                    case "name_asc":
                        $orderby = '(SELECT COUNT(*) FROM ' . $wpdb->prefix . 'st_order_item_meta 
                            WHERE st_booking_id = ' . $wpdb->posts . '.ID 
                            AND st_booking_post_type = "' . $post_type . '"
                            AND status IN ("completed", "confirmed", "paid")) ASC';
                        break;
                    case "name_desc":
...
$orderby = '(SELECT COUNT(*) FROM ' . $wpdb->prefix . 'st_order_item_meta 
                            WHERE st_booking_id = ' . $wpdb->posts . '.ID 
                            AND st_booking_post_type = "' . $post_type . '"
                            AND status IN ("completed", "confirmed", "paid")) DESC';
                        break;
                    case "rand":
                        $orderby = ' rand()';
                        break;
