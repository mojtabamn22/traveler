    ...
    class STRental extends TravelerObject
    {
        ...
        public function check_price_ajax(){
            ...
            if (!RentalHelper::check_day_cant_order($rental_origin, $check_in_tmp, $check_out_tmp, 1)) {
                wp_send_json(array('message' =>sprintf(__('This rental is not available from %s to %s.', 'traveler'), date_i18n('Y/m/d',strtotime($check_in)), date_i18n('Y/m/d',strtotime($check_out))) )); // Hamzeh
                die();
...
//                    ";
        ...
        function do_add_to_cart()
        {
            ...
            $check_out_tmp = date('Y-m-d', strtotime($check_out));
            if (!RentalHelper::check_day_cant_order($rental_origin, $check_in_tmp, $check_out_tmp, 1)) {
                STTemplate::set_message(sprintf(__('This rental is not available from %s to %s.', 'traveler'), date_i18n('Y/m/d',strtotime($check_in)), date_i18n('Y/m/d',strtotime($check_out))), 'danger'); // hamzeh
                $form_validate = false;
                return false;
            }
