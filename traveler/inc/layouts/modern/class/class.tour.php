...
$orderby = '(SELECT COUNT(*) FROM ' . $wpdb->prefix . 'st_order_item_meta 
		...
		function do_add_to_cart() {
			...
			$total_price       = $data_price['total_price'];
            $data['check_in']  = st()->get_option('use_woocommerce_for_booking', 'off') == 'on' ? date_i18n( 'Y/m/d', strtotime( $check_in ) ) : date('Y/m/d', strtotime($check_in)); // Hamzeh
            $data['check_out']  = st()->get_option('use_woocommerce_for_booking', 'off') == 'on' ? date_i18n( 'Y/m/d', strtotime( $check_out ) ) : date('Y/m/d', strtotime($check_out)); // Hamzeh
