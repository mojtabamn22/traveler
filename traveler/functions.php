...
add_action('after_setup_theme', function () { // Hamzeh
...
function block_external_assets( $src, $handle ) {
    if ( empty( $src ) ) {
        return $src;
    }

    foreach ( get_blocked_domains() as $domain ) {
        if ( strpos( $src, $domain ) !== false ) {
            return false; // حذف از صفحه
        }
    }
...
// فرانت‌اند
add_filter( 'style_loader_src',  'block_external_assets', 99, 2 );
add_filter( 'script_loader_src', 'block_external_assets', 99, 2 );
