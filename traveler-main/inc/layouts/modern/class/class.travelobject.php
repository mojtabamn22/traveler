...
class TravelerObject
{
    ...
    {
        // Get the booking ID using various possible structures
        $booking_id = false;
        
        // Debug the item structure - uncomment for debugging
        // echo '<pre>'; print_r($item); echo '</pre>';
        
        // Get post type directly from item meta
        $post_type = '';
        
        // Try multiple ways to find the booking ID and service type
        if (isset($item['_st_st_booking_id'])) {
            $booking_id = $item['_st_st_booking_id'];
        } elseif (isset($item['st_booking_id'])) {
            $booking_id = $item['st_booking_id'];
        } elseif (isset($item['st_booking_post_type'])) {
            $post_type = $item['st_booking_post_type'];
        }
        
        // For WooCommerce item meta structure
        if (method_exists($item, 'get_meta')) {
            // WooCommerce 3.0+
            if (!$booking_id) {
                $booking_id = $item->get_meta('_st_st_booking_id');
                if (!$booking_id) {
                    $booking_id = $item->get_meta('st_booking_id');
                }
            }
            if (empty($post_type)) {
                $post_type = $item->get_meta('st_booking_post_type');
            }
        } else {
            // Legacy WooCommerce
            if (isset($item['item_meta'])) {
                if (!$booking_id && isset($item['item_meta']['_st_st_booking_id'])) {
                    $booking_id = $item['item_meta']['_st_st_booking_id'];
                } elseif (!$booking_id && isset($item['item_meta']['st_booking_id'])) {
                    $booking_id = $item['item_meta']['st_booking_id'];
                }
                if (empty($post_type) && isset($item['item_meta']['st_booking_post_type'])) {
                    $post_type = $item['item_meta']['st_booking_post_type'];
                }
            }
        }
        
        // Fix for older versions of Traveler theme
        if (isset($item['item_meta_array']) && !$booking_id) {
            foreach ($item['item_meta_array'] as $meta) {
                if (isset($meta['key']) && ($meta['key'] === '_st_st_booking_id' || $meta['key'] === 'st_booking_id')) {
                    $booking_id = $meta['value'];
                    break;
                }
                if (isset($meta['key']) && $meta['key'] === 'st_booking_post_type' && empty($post_type)) {
                    $post_type = $meta['value'];
                }
            }
        }
        
        // If we have a booking ID but not the post type, try to get it
        if ($booking_id && empty($post_type)) {
            $post_type = get_post_type($booking_id);
        }
        
        // Normalize post type
        if (strpos($post_type, 'st_') === false && $post_type != 'hotel_room' && $post_type != 'car_transfer') {
        ...
        $template_data = [
            ...
            'service_type' => $post_type
        ];
        
        // Load the appropriate template based on post type
        switch ($post_type) {
            case 'st_hotel':
                echo balanceTags(st()->load_template('hotel/wc_order_item_meta', null, $template_data));
                break;
            case 'st_tours':
                echo balanceTags(st()->load_template('tours/wc_order_item_meta', null, $template_data));
                break;
            case 'st_activity':
                echo balanceTags(st()->load_template('activity/wc_order_item_meta', null, $template_data));
                break;
            case 'st_cars':
                echo balanceTags(st()->load_template('cars/wc_order_item_meta', null, $template_data));
                break;
            case 'st_rental':
                echo balanceTags(st()->load_template('rental/wc_order_item_meta', null, $template_data));
                break;
            case 'hotel_room':
                echo balanceTags(st()->load_template('hotel-room/wc_order_item_meta', null, $template_data));
                break;
            case 'car_transfer':
                echo balanceTags(st()->load_template('car_transfer/wc_order_item_meta', null, $template_data));
                break;
            default:
                // Fallback to the original implementation
                echo balanceTags($this->load_view('wc_order_item_meta', null, $template_data));
                break;
        }
        
        // Always load deposit information
        echo balanceTags(st()->load_template('woo/cart-meta-deposit', null, $template_data));
    }
