...
<div class="info-section">
    <ul>
        ...
        <li>
            ...
            <span class="value"><!-- Hamzeh -->
				<?php echo date_i18n('Y/m/d H:i A',$item['data']['check_in_timestamp']); ?>
				-
				<?php echo date_i18n('Y/m/d H:i A',$item['data']['check_out_timestamp']); ?></span>
        </li>
		<li>
...
<div class="total-section">
    <ul>
        ...
        if (isset($item['data']['deposit_money']) && count($item['data']['deposit_money']) && floatval($item['data']['deposit_money']['amount']) > 0){
            ...
            <?php if(class_exists('Sheyda\HamArz\Currency')) : ?>
                <li class="payment-amount">
                    <span class="label">معادل</span>
                    <?php
                        $currencies = (new \Sheyda\HamArz\Currency)->get_currencies( ['active' => true] );
                        if( $currencies ) {
                            $first_key = array_key_first($currencies);
            
                            $value = $currencies[$first_key]->value;
                            $total_rate = ($total_price * $value);
                        }
                    ?>
                    <span class="value"><?php echo TravelHelper::format_money_raw($total_rate, 'تومان', 0); ?></span>
                </li>
            <?php endif; ?>
        <?php
        } else { ?>
                ...
                <li class="payment-amount">
                    ...
                    <?php
                        ...
                        if( $currencies ) {
                            ...
                            $value = $currencies[$first_key]->value;
                            $total_rate = ($price_with_tax * $value);
                        }
                    ?>
                    <span class="value"><?php echo TravelHelper::format_money_raw($total_rate, 'تومان', 0); ?></span>
                </li>
