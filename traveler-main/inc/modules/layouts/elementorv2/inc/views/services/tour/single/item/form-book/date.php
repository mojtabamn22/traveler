<?php
    ...
    if(!empty(st_get_date_checkin_checkout_groupday_tour(TravelHelper::post_origin(get_the_ID(), 'st_tours'),  $time_checkin  ))){
        $timestamp_end = st_get_date_checkin_checkout_groupday_tour(TravelHelper::post_origin(get_the_ID(), 'st_tours'),$time_checkin)[0]['check_out'];
    ...
    $current_calendar = TravelHelper::get_current_available_calendar(TravelHelper::post_origin(TravelHelper::post_origin(get_the_ID(), 'st_tours'), 'st_tours'));
    $current_calendar_reverb = date('m/d/Y', strtotime($current_calendar));
?>
...
data-availability-date="<?php echo esc_attr($current_calendar_reverb); ?>">
    ...
    <div class="date-wrapper d-flex  justify-content-between align-items-center">
        <div class="check-in-wrapper">
            ...
            <div class="render check-in-render"><?php echo esc_html(date_i18n('Y/m/d',strtotime($start))); ?></div>
        </div>
    ...
    <input type="text" class="check-in-out-input"
            ...
            data-tour-id="<?php echo TravelHelper::post_origin(get_the_ID(), 'st_tours'); ?>" data-posttype="st_tours">
</div>
<?php
/*Starttime*/
...
$list_time = AvailabilityHelper::_get_starttime_tour_frontend_by_date(TravelHelper::post_origin(get_the_ID(), 'st_tours'),$current_calendar,$current_calendar);
?>
<div class="form-group form-more-extra st-form-starttime"
    ...
    <input type="hidden" data-starttime="<?php echo esc_attr($starttime_value); ?>"
        ...
        data-tourid="<?php echo TravelHelper::post_origin(get_the_ID(), 'st_tours'); ?>"
